...
void move_TB6612FNG(uint8_t * motordata)
{
  ...
  analogWrite(PWMA, 255);          // set speed motor A (left motor)
  ...
  analogWrite(PWMB, 255);          // set speed motor B (right motor)
  //analogWrite(PWMB, motordata[6]);          // set speed motor B (right motor)
 if ((motordata[3] == 4) &&(motordata[5] == 4)) 
 {
...
void run_TB6612FNG(uint8_t dir, uint8_t pin_in1, uint8_t pin_in2)
{
  ...
  uint8_t IN2;
  //digitalWrite(LAS,HIGH);
 // uint8_t IN3;  //Sets the value for the laser
